...
console.log('\n[3] POST /api/config/test-browserstack — auth + shape');
{
	...
	if (token) {
		...
		const after = JSON.parse(fs.readFileSync(path.join(ROOT, '.qase', 'config.json'), 'utf8'));
		ok(after.browserstackLastVerified && typeof after.browserstackLastVerified.ts === 'number', 'last-verified outcome persisted to config');
		ok(!JSON.stringify(after.browserstackLastVerified ?? {}).includes('tRuEje,27,&') && !/key["']?\s*:\s*"/i.test(JSON.stringify(after.browserstackLastVerified ?? {})), 'persisted record contains no credential material');
	}
}
