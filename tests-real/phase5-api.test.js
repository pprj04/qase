...
async function makeMissionViaApi(overrides = {}) {
  const { status, body } = await apiCall('POST', '/api/v1/missions', {
    ...
    type: 'qa',
    // Use the server's own origin (a first-class local practice target under
    // the M1-P4.1 target-URL boundary) — the old port 9876 never hosted anything
    // and is correctly rejected by the boundary as PORT_NOT_ALLOWED.
    targetUrl: overrides.targetUrl || 'http://localhost:5173/dashboard',
