...
describe('Phase 17 API', () => {
	...
	before(async () => {
		...
		// The gate runs 49 suites against the shared live server; a single
		// transient fetch failure here cancels all 16 subtests, so retry the
		// discovery call a few times with backoff before giving up.
		let last = null;
		for (let attempt = 0; attempt < 4; attempt += 1) {
			try {
				...
				last = new Error(`missions list returned ${status}`);
			} catch (err) {
				last = err;
			}
			await new Promise((r) => setTimeout(r, 2000 * (attempt + 1)));
		}
		assert.fail(`could not discover a completed mission after retries: ${last?.message ?? 'unknown'}`);
	});
