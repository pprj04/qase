...
const Project = {
	...
	properties: {
		...
		base_url: { type: 'string', description: 'Primary target base URL for this project.' },
		workspace_id: { type: 'string', description: 'Owning workspace id, when workspaces are in use.' },
		created_at: dt,
		updated_at: dt,
	},
};
...
const Mission = {
	...
	properties: {
		...
		source: { type: 'string', enum: MISSION_SOURCES, description: 'How the mission was created.' },
		project_id: { type: 'string', description: 'Owning project id.' },
		project_name: { type: 'string', description: 'Owning project name (joined; absent when the project was deleted).' },
		target_url: { type: 'string', description: 'Site under test.' },
		session_id: { type: 'string', description: 'Current/last execution session id, when linked.' },
		quality_score: { type: 'integer', minimum: 0, maximum: 100, description: 'Overall quality score set on completion.' },
		verdict: { type: 'string', enum: ['pass', 'pass_with_issues', 'fail'], description: 'Final verdict, once completed.' },
		release_ready: { type: 'boolean', description: 'Release-readiness flag set on completion.' },
...
const Session = {
	...
	properties: {
		...
		project_id: { type: 'string', description: 'Owning project id, when set.' },
		project_name: { type: 'string', description: 'Owning project name (joined).' },
		target_url: { type: 'string', description: 'Site under test.' },
		mission_id: { type: 'string', description: 'Mission this session executes, when linked.' },
		mission_name: { type: 'string', description: 'Linked mission name (joined).' },
...
const Finding = {
	...
	properties: {
		...
		primary_category: { type: 'string', enum: CATEGORIES, description: 'Normalized bug category.' },
		secondary_categories: { type: 'array', items: { type: 'string', enum: CATEGORIES }, maxItems: 3 },
		reproducibility: { type: 'string', enum: [...REPRODUCIBILITIES, 'confirmed', 'unconfirmed', 'intermittent'] },
		confidence: { type: 'number', minimum: 0, maximum: 1, description: 'Deterministic evidence-based confidence score.' },
		url: { type: 'string', description: 'Where the bug was observed.' },
		...
		session_title: { type: 'string', description: 'Linked session title (joined).' },
		mission_id: { type: 'string' },
		mission_name: { type: 'string', description: 'Linked mission name (joined).' },
		is_duplicate: { type: 'boolean' },
		duplicate_of: { type: 'string', description: 'Canonical finding id when this record is a duplicate.' },
...
const TestCase = {
	...
	properties: {
		...
		project_id: { type: 'string' },
		project_name: { type: 'string', description: 'Owning project name (joined).' },
		workflow_id: { type: 'string', description: 'Workflow the case was generated from.' },
		suite_id: { type: 'string', description: 'Suite the case belongs to.' },
...
const Suite = {
	...
	properties: {
		...
		project_id: { type: 'string' },
		project_name: { type: 'string', description: 'Owning project name (joined).' },
		parent_id: { type: 'string', description: 'Parent suite id for tree structures.' },
...
const RegressionRun = {
	...
	properties: {
		...
		schedule_name: { type: 'string', description: 'Owning schedule name (joined).' },
		project_id: { type: 'string' },
		project_name: { type: 'string', description: 'Owning project name (joined).' },
		target_url: { type: 'string' },
		trigger: { type: 'string' },
...
const FixValidationRun = {
	...
	properties: {
		...
		finding_id: { type: 'string', description: 'Finding under validation.' },
		finding_title: { type: 'string', description: 'Snapshot title of the finding under validation.' },
		mission_id: { type: 'string' },
		project_id: { type: 'string' },
		project_name: { type: 'string', description: 'Owning project name (joined).' },
		status: { type: 'string', enum: FIX_RUN_STATUSES, description: 'Run lifecycle.' },
		fix_status: { type: 'string', enum: FIX_STATUSES, description: 'Deterministic fix outcome once completed.' },
...
const KnowledgePattern = {
	...
	properties: {
		id: id(),
		pattern: { type: 'string', description: 'The learned assertion/pattern text.' },
		category: { type: 'string' },
		status: { type: 'string', enum: KNOWLEDGE_STATUS },
		confidence: { type: 'number', minimum: 0, maximum: 1, description: 'Current confidence after decay/validation.' },
