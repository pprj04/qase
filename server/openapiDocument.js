...
const Mission = {
	...
	properties: {
		...
		finding_count: { type: 'integer', description: 'Current canonical finding count; merged/duplicate records are excluded.' },
		finding_snapshot_count: nullable({ type: 'integer', description: 'Canonical count represented by the embedded mission/session snapshot.' }),
		finding_count_semantics: nullable({ type: 'string', enum: ['canonical_current'] }),
		execution_outcome: nullable({ type: 'string', enum: ['pending', 'queued', 'running', 'completed', 'failed', 'blocked', 'partial', 'cancelled'] }),
		outcome_reason: nullable({ type: 'string' }),
		report_available: { type: 'boolean', description: 'Whether a persisted structured report exists; independent of execution outcome.' },
...
const Session = {
	...
	properties: {
		...
		finding_count: { type: 'integer', description: 'Current canonical finding count; merged/duplicate records are excluded.' },
		finding_snapshot_count: nullable({ type: 'integer', description: 'Canonical count represented by the embedded run snapshot.' }),
		finding_count_semantics: nullable({ type: 'string', enum: ['canonical_current'] }),
		execution_outcome: nullable({ type: 'string', enum: ['pending', 'queued', 'running', 'completed', 'failed', 'blocked', 'partial', 'cancelled'] }),
		outcome_reason: nullable({ type: 'string' }),
		report_available: { type: 'boolean', description: 'Whether a persisted structured report exists; independent of execution outcome.' },
